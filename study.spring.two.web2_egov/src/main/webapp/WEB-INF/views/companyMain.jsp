    ...
    <td><table width="640" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          ...
          <form action="search2.do?pageNum=1" method="post" id="keywordForm">
              ...
              <option value="companyNo" <c:out value="${map.searchOption eq 'companyNo'?'selected':'' }"/> >회사고유번호</option>
              <option value="daepyosName" <c:out value="${map.searchOption eq 'daepyosName'?'selected':'' }"/> >대표이름</option>
            </select>
              ...
              <tr> 
				...
				<c:forEach var="tmp" items="${map.testList }">
                    <tr> 
                      ...
                      <td width="91" align="center">${tmp.daepyoType }</td>
...
function deleteUpdate(index){
	if($("input[name=check_sub]").is(":checked")==0){
		alert("삭제 혹은 수정을 하시려면 1개 이상 선택하세요!");
			...
			if(index==1){
				if(confirm("삭제하시겠습니까?")){
					document.updateDelete.action = "companyDelete.do";
				}else{
					return false;
				}
			}
			if(index==2){
				if(confirm("수정하시겠습니까?")){
					document.updateDelete.action = "companyUpdateform.do";
				}else{
					return false;
				}
			}
