  ...
  <tr> 
    <td><table width="640" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          ...
          <form action="search.do?pageNum=1" method="post" id="keywordForm">
          	<select name="searchOption" class="INPUT">
              <option value="all" <c:out value="${map.searchOption eq 'all'?'selected':'' }"/> >전체</option>
              <option value="kor_name" <c:out value="${map.searchOption eq 'kor_name'?'selected':'' }"/> >이름</option>
              <option value="gender" <c:out value="${map.searchOption eq 'gender'?'selected':'' }"/> >성별</option>
              <option value="tech" <c:out value="${map.searchOption eq 'tech'?'selected':'' }"/> >기술등급</option>
            </select>
        ...
        <tr> 
          <td><table width="640" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr> 
				...
				<table width="640" border="0" cellspacing="0" cellpadding="0">		
					<tr> 
                      ...
                      <td width="153" align="center">주민등록번호</td>
				...
				<c:forEach var="tmp" items="${map.testList }">
                    <tr> 
                      <td width="35" height="20" align="center"><input type="checkbox" name="check_sub" value="${tmp.no }"></td>
                      <td width="85" align="center">
                      	<%-- <a href="userSelectOne&keyword=${map.keyword}"></a> --%>
                      		${tmp.kor_name }
                      	
                      </td>
                      <td width="153" align="center">${tmp.jumin1 } - ${tmp.jumin2 }</td>
                      <td width="91" align="center">${tmp.gender }</td>
                      <td width="91" align="center">${tmp.tech }</td>
                      <td width="91" align="center">${tmp.work_state }</td>
                      <td width="94" align="center">${tmp.work_type }</td>
                    </tr>
                    <tr> 
							...
							<ul class="pagination">
								<c:choose>
									<c:when test="${startPageNum ne 1 }">
										<li>
											<a href="main.do?pageNum=${startPageNum - 1 }&keyword=${map.keyword }" aria-label="Previous">
								...
								<c:forEach var="i" begin="${startPageNum }" end="${endPageNum }">
									<li <c:if test="${i eq pageNum}">class="active"</c:if>><a href="main.do?pageNum=${i }&keyword=${map.keyword }">${i }</a></li>
								</c:forEach>
								<c:choose>
									<c:when test="${endPageNum lt totalPageNum }">
										<li>
											<a href="main.do?pageNum=${endPageNum + 1 }&keyword=${map.keyword }" aria-label="Previous">
              ...
              <tr align="center" bgcolor="F8F8F8"> 
                <td height="26" align="right" bgcolor="F8F8F8" style="padding-right:10"><img src="${pageContext.request.contextPath }/resources/image/all_icon.gif" width="11" height="11" align="absmiddle"> 
                <a onclick="deleteUpdate(5)" href="javascript:">조회</a> <img src="${pageContext.request.contextPath }/resources/image/all_icon.gif" width="11" height="11" align="absmiddle"> 
...
function deleteUpdate(index){
	if($("input[name=check_sub]").is(":checked")==0){
		alert("삭제 혹은 수정을 하시려면 1개 이상 선택하세요!");
			...
			if(index==5){
				if(confirm("조회하시겠습니까?")) {
					document.updateDelete.action = "userSelectOne.do";
				} else {
					return false;
				}
			}
